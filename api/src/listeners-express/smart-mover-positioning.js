...
export default function registerSmartMoverPositioning(app) {
  ...
  app.post("/api/execute-positioning-v4", async (req, res) => {
    try {
      ...
      // gard�s pour compatibilit� avec le front, m�me si le script test ne les utilise pas
      ...
      if (!worldId || !boardId) {
        return res.status(400).send("Missing worldId or boardId");
      }

      if (state.running) {
        return res.status(409).send("Positioning already running");
      }
      ...
      pushLog(
        `CFG componentName="${componentName}" projectField="${projectField}" dateField="${dateField}"`
      );
      pushLog(`SCRIPT=${SCRIPT}`);
      pushLog(`CMD: node ${SCRIPT} --worldId ${worldId} --boardId ${boardId} --auto`);
