...
BATCH_SIZE = 5  # Traiter 5 objets à la fois
...
class TicketManager:
    ...
    def update_board_objects_batch(self, board_id, data, batch_size=BATCH_SIZE):
        """Mettre à jour les objets dans un board par lots"""
        print(f"\n📝 Mise à jour du board {board_id} par lots de {batch_size}...")
        ...
        print(f"   📊 {total_objects} objets à traiter en {(total_objects + batch_size - 1) // batch_size} lots")
        ...
        for i in range(0, total_objects, batch_size):
            ...
            print(f"\n   📦 Lot {batch_number}/{total_batches} - {len(batch_objects)} objets")
            
            # Créer une structure de données pour ce lot
            batch_data = copy.deepcopy(data)
            batch_data["VW"]["objects"] = batch_objects
            
            # Afficher les noms des objets dans ce lot
            object_names = []
            for obj in batch_objects:
                try:
                    if obj.get("object") and obj["object"].get("data"):
                        name = obj["object"]["data"].get("name", "Sans nom")
                        object_names.append(name)
                except:
                    object_names.append("Sans nom")
            
            print(f"      📋 Objets: {', '.join(object_names)}")
            ...
            if success:
                ...
                print(f"      ❌ Échec du lot {batch_number}")
    ...
    def modify_context_and_copy_to_response(self, modifications):
        ...
        print(f"\n🔄 Processus: Modifier Context → Copier vers Response (par lots)")
        ...
        print(f"\n🔄 5. Mise à jour du board context par lots...")
        context_update_success = self.update_board_objects_batch(BOARD_CONTEXT_ID, modified_data)
        ...
        print(f"\n📋 8. Copie vers le board response par lots...")
        response_update_success = self.update_board_objects_batch(BOARD_RESPONSE_ID, modified_data)
...
def main():
    ...
    print("🔄 Mode: Modifier Context → Copier vers Response (traitement par lots)")
    print("=" * 85)
    ...
    try:
        ...
        if success:
            ...
            print(f"   3️⃣  Traitement par lots de {BATCH_SIZE} pour éviter l'erreur 413")
            ...
            print(f"🚫 Aucune duplication : boards nettoyés avant recréation")
        else:
