...
void AbstractGeneticAlgo::write(const std::string& filename) const {
	...
	// ===== NOUVEL EN-TÊTE =====
	fichier << "id_work_element ; workelement_name ; attributepriority ; gateid ; date_update ; load_engage ; duration_engage ; fte_allocated_engage ; percent_accomplished ; New_Team_id ; Old_team_id ; start_date ; end_date ; due_start_date ; due_date" << std::endl;
	...
	for (int i = 0; i < _data.nbJobs(); i++) {
		...
		try {
			...
			int end_period = start_period + dur_period;
