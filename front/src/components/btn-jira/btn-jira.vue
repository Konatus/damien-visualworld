<template>
    <button class="btn-jira" v-if="jiraIssueList" @click="jiraIssues">
        <svg
            width="32"
            viewBox="0 0 32 32"
...
import JiraIssueList from "../jira-issue-list/jira-issue-list.vue";
export default {
    name: "BtnJira",
    inject: ["$view"],
    computed: {
        jiraIssueList() {
            return !!this.$store.getters["boardAlive/byId"](this.$view.boardId)
                ?.data?.jira;
        },
    },
    methods: {
        jiraIssues() {
            this.$modal.show(
                ...
                app.modal.parameters_jira_issue_list
            );
        },
    },
};
